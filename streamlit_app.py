...
def process_data(sales_file, user_file, att_file, cov_file, cc_file, ful_file):
    ...
    # Fallbacks in case they absolutely don't exist
...
if all([f_sales, f_user, f_att, f_cov, f_cc, f_ful]):
    with st.spinner("Processing data, please wait..."):
        try:
            ...
            # Common Number Configs for proper formatting
            ...
            # ==========================================
            # CASCADING FILTERS (Directly Visible)
            # ==========================================
            ...
            if selected_emp:
                ...
                if not months:
                    ...
                    # FORCE PANDAS DTYPES TO GUARANTEE RIGHT-ALIGNMENT IN STREAMLIT
                    ...
                    df_trend["Lines Billed (Categories)"] = pd.to_numeric(df_trend["Lines Billed (Categories)"])
